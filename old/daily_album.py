...
#important note --- will not work on the last day of a month
#this is a known error, will be fixed later
...
previous = int(day) - 1
prior_day = str(previous)
month = datetime.date.today().strftime("%B")
year = datetime.date.today().strftime("%Y")

prior_date = month + ' ' + prior_day + ' ' + year
    ...
    with open('scrape-results.txt', 'a') as g:
        for line in f:
            ...
            try:
                ...
                print prior_day
                if 'hrs' in date_published:
                    date_published = month + ' ' + day + ' ' + year
                elif date_published == prior_date:
                    pass
                else:
                    print('all done')
                    break
